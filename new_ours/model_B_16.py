# attentional_vlm_model.py (最终版 for ViT-B-16)
import torch
...
class UNetDecoder(nn.Module):
    """
    # <--- 已更新为 ViT-B-16 配置 --->
    修改后的UNet解码器，以适配ViT-B-16的768维特征。
    """
    ...
    def __init__(self, out_channels=1):
        ...
        # 适配768维bottleneck的解码器结构
        self.upconv4 = nn.ConvTranspose2d(768, 384, kernel_size=2, stride=2)
        self.decoder4 = conv_block(768, 384)  # cat(384, 384) -> 768
        self.upconv3 = nn.ConvTranspose2d(384, 192, kernel_size=2, stride=2)
        self.decoder3 = conv_block(384, 192)  # cat(192, 192) -> 384
        self.upconv2 = nn.ConvTranspose2d(192, 96, kernel_size=2, stride=2)
        self.decoder2 = conv_block(192, 96)  # cat(96, 96) -> 192
        self.upconv1 = nn.ConvTranspose2d(96, 64, kernel_size=2, stride=2)
...
class VLM_UNet(nn.Module):
    def __init__(self, clip_model, out_channels=1, unfreeze_backbone=False):
        ...
        # <--- 已更新为 ViT-B-16 配置 --->
        vision_width = 768  # Transformer输出的网格特征维度
        text_width = 512  # encode_text输出的特征维度

        # 增加投影层来“压缩”Skip-Connection的通道维度
        self.skip_proj_4 = nn.Conv2d(vision_width, 384, kernel_size=1)
        self.skip_proj_3 = nn.Conv2d(vision_width, 192, kernel_size=1)
        self.skip_proj_2 = nn.Conv2d(vision_width, 96, kernel_size=1)
        self.skip_proj_1 = nn.Conv2d(vision_width, 64, kernel_size=1)

        # 关键的投影层：将512维文本特征映射到768维
    ...
    def forward(self, image, text_tokens, return_features=False):
        ...
        enc1_unproj = F.interpolate(bottleneck, size=(bottleneck.shape[2] * 16, bottleneck.shape[3] * 16),
                                    mode='bilinear', align_corners=False)
        enc_features = {
            'enc4': self.skip_proj_4(enc4_unproj),  # 768 -> 384 channels
            'enc3': self.skip_proj_3(enc3_unproj),  # 768 -> 192 channels
            'enc2': self.skip_proj_2(enc2_unproj),  # 768 -> 96 channels
            'enc1': self.skip_proj_1(enc1_unproj),  # 768 -> 64 channels
        }
...
class AttentionalVLM_FlowMatching(nn.Module):
    # <--- 已更新为 ViT-B-16 配置 --->
    def __init__(self, embed_dim=768, num_heads=8, hidden_channels=256):
        super().__init__()
...
class FullSegmentationModel(nn.Module):
    # <--- 已更新为 ViT-B-16 配置 --->
    def __init__(self, clip_model_name='ViT-B-16', pretrained='datacomp_l_s1b_b8k', unfreeze_backbone=False,
                 flow_steps=10):
        super().__init__()
